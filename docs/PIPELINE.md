...
`token_hash` у самозаписанной машины синтетический (`self:<uuid>`): она ходит своим
`mch_`-токеном, а не выданным `rc_`, и подобрать этот хеш нечем — он не хеш токена.
Уникальность UUID держит частичный индекс среди неотозванных, поэтому отозванную
машину можно завести заново.
