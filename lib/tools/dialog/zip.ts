 ...
 * экспорта титров это недостижимо, а для озвучки — почти: час несжатого моно в
 * 32 кГц весит около 230 МБ, и упереться в 4 ГБ можно только десятком дорожек
 * целого фильма. Проверка всё равно стоит две строки — молча выдать битый архив
 * хуже, чем отказаться.
 */

/**
 * Файл в архиве: либо текст, либо готовые байты.
 *
 * Разделение нужно из-за озвучки: WAV — двоичный файл, и провести его через
 * строку без потерь нельзя. Тип написан так, что одно из двух полей обязательно,
 * а оба сразу — ошибка ещё при сборке.
 */
export type ZipEntry =
  | {
      /** Имя внутри архива. Разделитель — только косая черта. */
      name: string
      /** Содержимое текстом; пишется в UTF-8. */
      text: string
      bytes?: undefined
    }
  | {
      name: string
      text?: undefined
      /** Содержимое байтами — звук, картинка, что угодно двоичное. */
      bytes: Uint8Array
    }
...
export function buildZip(entries: ZipEntry[], modified: Date): Uint8Array {
  ...
  const files = entries.map((entry) => {
    ...
    const data = entry.bytes !== undefined ? entry.bytes : encoder.encode(entry.text)
