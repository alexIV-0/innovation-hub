...
9. **Пользовательские строки в `/account` и `/admin` — только через словарь** (`useI18n()` / `useAdminI18n()`), ru + en.
...
components/
  ...
  account/
    ...
    workspace/            страница проектов: контекст данных, топбар, колонка проектов,
                          ...
                          нижняя панель, контекстное меню, режимы full/simple, мобильный вид
...
- Провайдер: `I18nProvider` в `components/account/i18n.tsx`, монтируется внутри `WorkspaceShell` → покрывает и `/account`, и `/admin`.
- Хуки: `useI18n()` → `{ t, lang, setLang }`; для админки — `useAdminI18n()` → `t` (`components/admin/admin-dict.ts`), плюс `tf()` для строк с подстановкой.
- Язык хранится в `localStorage` под ключом `ffworks-lang`, переключатель RU/EN — в подвале сайдбара.
- Словарь — плоский объект `{ ru: {...}, en: {...} }`; ключи `camelCase`, сгруппированы комментариями по смыслу.
- Типы: `DictKey` используется в конфигах (например, `nav-config.ts` хранит `labelKey`, а не готовую строку) — так навигация остаётся локализуемой.
- Хелперы форматирования тоже в `i18n.tsx`: `formatBalance(cents, lang)`, `avatarInitials()`, `greetingForHour()`. Форматирование дат — `toLocaleDateString(lang === "ru" ? "ru-RU" : "en-US", …)`.

**Добавляя строку в `/account` или `/admin`:** сначала ключ в оба словаря, потом использование. Хардкод русского/английского текста в JSX этих зон — ошибка ревью. Публичный сайт сейчас англоязычный и словаря не имеет.
...
- [ ] Строки локализованы (для `/account`, `/admin`)
