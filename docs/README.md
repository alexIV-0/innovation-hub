...
**[DB_SCHEMA.html](./DB_SCHEMA.html) — схема базы, снятая с живого сервера.**
...
браузере. Это снимок, а не исходник: DDL по-прежнему в `db/schema.sql` и
`db/migrations/`. Пересобрать — `npm run db:schema-doc`.
