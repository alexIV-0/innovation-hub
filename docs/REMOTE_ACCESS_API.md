...
Ответ на `heartbeat` несёт `vaultRevision` — версию сейфа секретов. Разошлась с
вашей — сходите за ключами (`vendorKeys`): так отзыв ключа доезжает за полминуты,
а не по истечении срока локальной копии.

### Сейф (ключи внешних сервисов)

| action | Назначение |
|---|---|
| `vendorKeys` | Ключи сервисов для этой машины. Спрашивать ПЕРЕД задачей и только по нужным ей сервисам; в `known` передавать версии из локального сейфа — совпало, ключ не поедет по сети |
| `vendorUsage` | Потребление по задаче: **единицы, не деньги**. Слать сразу после ответа вендора, не дожидаясь `taskDone` |

Разбор — [VENDOR_SERVICES_PLAN.md](./VENDOR_SERVICES_PLAN.md), контракт для
программы — `fs.manager.tauri/ideasAndTest/VENDOR_KEYS_CONTRACT.md`.
...
| action | Назначение |
|---|---|
| `machinePing` | «Я на связи», без запроса задачи — звать на своём пульсе |
| `claimTask` | Атомарно забрать следующую задачу; пустая очередь — `task: null` |
| `taskProgress` | Двинуть шаг и продлить аренду (15 мин) |
| `taskDone` | Завершить; идемпотентно по `taskId` |
| `taskFailed` | Пометить упавшей; `payload` сохраняется |
| `releaseTask` | Вернуть в очередь при аварийном стопе |

Модель — pull, судья при захвате — `FOR UPDATE SKIP LOCKED`. Видимость задач
наследует роль токена: админский видит общую очередь, обычный — только проекты своего
владельца.

Те же операции доступны десктопу под его `mch_`-токеном:
`POST /api/storage/v1/queue` с полем `action` (`claim` | `progress` | `done` |
`failed` | `release`). Второй токен заводить не нужно — машина опознаётся полем
`machineUuid`, и сайт заводит для неё строку сам.

### Настройки

| action | Назначение |
|---|---|
| `getSettings` | Общие словари: типы файлов, цвета нод и данных, маски путей |
| `putSettings` | Запись словарей; `baseRevision` устарела — `409` с текущим документом |

Словари общие на всю установку, а не на проект. Пути к программам и папкам
материалов машинно-локальные — их доменов здесь нет вовсе.

Отдельный поллинг не нужен: текущая ревизия приезжает в поле `settingsRevision`
ответа `delta`. Правила слияния — [SETTINGS_SYNC.md](./SETTINGS_SYNC.md).
